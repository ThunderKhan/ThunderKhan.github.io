...
const FRAGMENT_SHADER = `// "Halftone" — made with the 21st.dev Shader Builder
// Packed WebGL1 uniforms (the shader exposes readable u_* aliases as macros):
//   u_colors[8] (first 4 used)
//   vec3(0.043, 0.063, 0.149)
//   vec3(0.239, 0.275, 0.910)
//   vec3(0.694, 0.549, 1.000)
//   vec3(1.000, 0.839, 0.906)
//   u_scene = vec4(canvas width, canvas height, seconds * 1.43, 4.0)
//   u_shape = vec4(1.48, 0.88, 0.50, 0.00)
//   u_surface = vec4(2.40, 0.91, 0.00, 1.00)
//   u_finish = vec4(0.00, 0.00, 0.000, 0.06)
//   u_transform = vec4(2926.0, 0.00, 0.00, 0.0)
...
vec3 linToOklab(vec3 c) {
  ...
  return vec3(
    ...
    0.0259040371 * l + 0.7827711662 * m - 0.8086757660 * s);
}
...
function compileShader(gl: WebGLRenderingContext, type: number, source: string) {
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    console.warn('Ambient shader compilation failed:', gl.getShaderInfoLog(shader))
...
function HalftoneShader() {
  ...
  useEffect(() => {
    ...
    const flatColors = new Float32Array(8 * 3)
    COLORS.forEach((color, i) => flatColors.set(color, i * 3))
    gl.uniform3fv(colors, flatColors)
    gl.uniform4f(shape, 1.48, 0.88, 0.5, 0.0)
    gl.uniform4f(surface, 2.4, 0.91, 0.0, 1.0)
    gl.uniform4f(finish, 0.0, 0.0, 0.0, 0.06)
    ...
    const render = (now: number) => {
      ...
      gl.uniform4f(scene, canvas.width, canvas.height, elapsed * 1.43, 4.0)
      gl.uniform4f(space, 0.0, 0.0, pointer.x, pointer.y)
      // The supplied preset had cursor interaction disabled. We intentionally
      // enable a restrained swirl for desktop pointers so the pattern bends
      // locally without turning the background into a cursor-follow gimmick.
      gl.uniform4f(cursor, pointer.presence, 2.0, 0.36, 0.42)

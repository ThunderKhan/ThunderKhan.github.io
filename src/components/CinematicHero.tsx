...
export function CinematicHero() {
  ...
  // Scene one: descend through the code singularity before arriving at the workspace.
  ...
  // Scene two: the abstract opening resolves into the human/workspace scene.
  ...
  const skillOpacities = [skill1Opacity, skill2Opacity, skill3Opacity, skill4Opacity]
  const skillStyles = skillOpacities.map((opacity) => ({
    opacity,
    y: useTransform(opacity, [0, 1], [14, 0]),
    scale: useTransform(opacity, [0, 1], [0.94, 1]),
  }))
  ...
  const terminalOpacities = [terminal1Opacity, terminal2Opacity, terminal3Opacity, terminal4Opacity]
  const terminalStyles = terminalOpacities.map((opacity) => ({
    opacity,
    x: useTransform(opacity, [0, 1], [-8, 0]),
  }))
        ...
        <div className="relative mx-auto h-full w-full max-w-7xl px-5 sm:px-8 lg:px-12">
          ...
          >
            ...
            <div className="mt-9 flex flex-wrap items-center gap-3">
              <a href="#projects" className="group inline-flex min-h-11 items-center gap-2 rounded-full bg-white px-6 py-2.5 text-sm font-semibold text-black shadow-[0_12px_40px_rgba(255,255,255,0.12)] transition-all hover:-translate-y-0.5 hover:shadow-[0_16px_50px_rgba(255,255,255,0.18)]">
                Enter my work
                <ArrowRight size={16} aria-hidden="true" className="transition-transform group-hover:translate-x-0.5" />
              </a>
              <a href={site.resume} download className="inline-flex min-h-11 items-center gap-2 rounded-full border border-white/30 bg-black/25 px-6 py-2.5 text-sm font-medium text-white backdrop-blur-md transition-colors hover:border-white/60 hover:bg-black/40">
          ...
          <m.div
            className="pointer-events-none absolute inset-x-5 top-[45%] text-center sm:inset-x-8 lg:inset-x-12"
            style={reducedMotion ? { opacity: 0 } : { opacity: transitionLabelOpacity, y: transitionLabelY }}
          >
          ...
          >
            <div className="flex flex-wrap gap-2.5 sm:gap-3">
              {skillSignals.map((signal, index) => (
                <m.span key={signal} style={reducedMotion ? undefined : skillStyles[index]} className="rounded-full border border-white/20 bg-black/30 px-4 py-2 font-mono text-[11px] tracking-[0.18em] text-white/85 uppercase backdrop-blur-xl sm:text-xs">
                  {signal}
                </m.span>
              ))}
            </div>
          ...
          >
            <div className="mb-3 flex items-center gap-2">
              <span className="size-2 rounded-full bg-white/45" /><span className="size-2 rounded-full bg-white/30" /><span className="size-2 rounded-full bg-white/20" />
